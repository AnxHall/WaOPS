...
| GET | `/api/v1/rate-limits` | Métrica de rate limiting: 429/min por classe de rota (janela 60s, contadores Redis `rl:stats:429:*` escritos pelo middleware, TTL 1000s) | User JWT | `tenants.read` | JWT/TenantContext | — | — | `{window_seconds:60, classes:[{routeClass, windowStart, limited}]}` (auth/api/realtime) | 401; 403 | — | Dashboard (card 429/min) | validado ao vivo + E2E rate-limit |
| GET | `/api/v1/rate-limits/history` | Série 429/min dos últimos 15 minutos por classe de rota (15 buckets de 60s, ordem cronológica, fonte Redis `rl:stats:429:*` TTL 1000s) | User JWT | `tenants.read` | JWT/TenantContext | — | — | `{window_seconds:60, points:15, classes:[{routeClass, limited:[{windowStart, limited}]}]}` | 401; 403 | — | Dashboard (sparkline do card 429/min) | E2E HM05 (429s reais nos buckets) |
